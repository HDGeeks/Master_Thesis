...
\begin{enumerate}
    ...
    \item \textbf{Titles versus abstracts diverge in opposite directions depending on the model, at least under this backend.} Qwen3-8B here is consistent with the earlier local run: titles slightly outperform abstracts. Claude shows the opposite pattern, with abstracts clearly outperforming titles. At the time this looked like evidence that whether additional context helps depends on the specific model. That conclusion needed revising once Qwen3-8B was later run on its native bf16 precision instead of a quantized backend, see Section~\ref{sec:model-size-setup}: the direction flips, and titles versus abstracts turns out to be sensitive to backend and precision for this model, not a stable per-model property either.
\end{enumerate}
