...
\title{\sffamily\vspace{-2em} Zero-Shot Topic Labeling: Extensions and Model Comparison\\[0.3em]\large Supervisor Meeting - Experiment-1}
...
\begin{tabular}{llccc}
\toprule
Size & Field & bf16 (default) & 4-bit & Difference \\
\midrule
...
\FloatBarrier
\section{Titles vs.\ Abstracts}

\begin{itemize}
    \item Abstracts beat titles in every single run tested, across all five Qwen3.5 sizes, Qwen3-8B, and Claude Haiku 4.5, no exceptions.
    \item The gap is small and fairly stable, roughly 1--3 points, smallest for the 27B model (+1.0 pt) and largest for Claude Haiku 4.5 (+3.1 pts). More input text helps a little across the board, but it never changes which model or size wins.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Extension 1 (matching fix):} mainly moves the hallucination column, not the model's actual answers. 4.1\% of Qwen3-8B title answers were wrongly counted as hallucination due to fragile exact-string matching. The fix recovered most of them into success. 
    \item \textbf{Extension 2 (newer models):} mainly collapses hallucination, from 25--33\% (paper) to under 4\% for every newer model tested, while success rises 17--30 points and misclassified stays roughly flat or rises slightly. 
