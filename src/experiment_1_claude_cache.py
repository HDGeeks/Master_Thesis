...
Same single-call, either-or (title or abstract) structure as
experiment_1_claude.py, with prompt caching added. The instructions and
targets_list vocabulary are identical for all 2500 calls, so they go in a
cached system block. Only the title or abstract text is sent fresh in the
user message each time, that is the only part that actually changes.

Zero-shot: no extended thinking, no examples, just the cached instructions
plus one line of document text and the model's direct answer.
...
RESULTS_PATH = os.path.join(
    ...
    f"experiment_1_claude_cache_{INPUT_FIELD}_{RUN_TIMESTAMP}_results.json",
)
...
def build_system_prompt(vocabulary):
    """Everything that stays identical across all 2500 calls: the task
    instructions and the full targets_list vocabulary. This is the part
    that gets cached, so it only gets billed at full price once."""
    ...
    return (
        ...
        f"{field_label} the user gives you.\n"
...
def ask_model(client, system_prompt, input_text):
    """Run the model once. The system block is marked as cacheable, so
    only the input_text (title or abstract) is new on every call.
        ...
        system=[
            {
                "type": "text",
                "text": system_prompt,
                "cache_control": {"type": "ephemeral"},  # cache everything above, reused every call
            }
        ],
...
def main():
    ...
    for i, doc in enumerate(documents, start=1):
        ...
        raw_answer, usage = ask_model(client, system_prompt, input_text)
...
def print_summary(results):
    ...
    total_cache_reads = sum(r["cache_read_input_tokens"] for r in results)
    total_cache_writes = sum(r["cache_creation_input_tokens"] for r in results)
    print(f"\ncache reads (discounted tokens): {total_cache_reads}")
    print(f"cache writes (first-call setup tokens): {total_cache_writes}")
